...
class _ConfirmPaymentState extends State<ConfirmPayment> {
  ...
  _createNewDonation() async {
    ...
    if (widget.methodId == 1) {
      ...
      paymentMode = widget.countryCode == "ug" ? "flexipay" : "other";
      phone = phoneController.text;
    } else if (widget.methodId == 2) {
      paymentMode = "card";
      phone = phoneController.text;
    } else if (widget.methodId == 3) {
      paymentMode = "bank_teller";
      phone = widget.memberPhone;
    } else {
    ...
    if (phone.length < 9) {
      showError(
          "Please enter a valid phone number ${widget.methodId} - $phone");
      return;
    }
    setState(() {
      _isLoading = true;
    ...
    var data = {
      'church_code': widget.churchCode.toString(),
      'telephone': phone,
      'currency': widget.currency,
      'mode': paymentMode,
      'code': memberCode.toString(),
      'fee_type': json.decode(widget.donationList)
    };
    if (kDebugMode) {
      print(data);
    }
    ...
    final prefs = await SharedPreferences.getInstance();
    var response = await httpService.postAppData(data, countryCodeUrl,
        'v2/church/payment?lang=${prefs.getString("currentLang")!}&v=${prefs.getString("version")!}');
                ...
                Column(
                  children: [
                    ...
                    Padding(
                      ...
                      child: Text(
                        "$countryCurrency ${widget.totalAmount.toStringAsFixed(0)}",
                        textAlign: TextAlign.center,
                        style: GoogleFonts.poppins(
                            color: greenColor,
                      ...
                      children: [
                       Text("Coming Soon"),
                        InkWell(
                          onTap: () {
                            showError(
                                "Payment failed, unhandled error occurred");
                          },
                          child: ActiveButton(
                            icon: Icons.check,
                                    ...
                                    children: [
                                      Row(
                                        ...
                                        children: [
                                          ...
                                          Text(
                                            widget.countryCode == "ug"
                                                ? "FlexiPay"
                                                : translate(
                                                    "app_txt_momo_payment"),
                                            style: GoogleFonts.poppins(
                                                color: blackColor,
                                                fontWeight: FontWeight.w400,
                                                fontSize: 15),
